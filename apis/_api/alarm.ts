...
export const getAlarm = async ({ page, size, isRead }: getAlarmParams) => {
  const { data } = await fetcher.get(
    `/v1/notification?page=${page}&size=${size}&isRead=${isRead}`
  );
...
export const readAlarm = async (alarmId: number) => {
  const { data } = await fetcher.patch(`/v1/notification/${alarmId}`);

  return data;
};

export const getExistIsNotReadAlarm = async () => {
  const { data } = await fetcher.get(`/v1/notification/exist`);
